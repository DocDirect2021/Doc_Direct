...
public class ScanFragment extends Fragment {
    ...
    if (isCropMode) {
      viewHolder.sourceFrame.post(new Runnable() {
        @Override public void run() {
          Bitmap scaledBitmap =
              ImageResizer.scaleBitmap(takenPhotoBitmap, viewHolder.sourceFrame.getWidth(),
                  viewHolder.sourceFrame.getHeight());
          viewHolder.sourceImageView.setImageBitmap(scaledBitmap);

          Bitmap tempBitmap =
              ((BitmapDrawable) viewHolder.sourceImageView.getDrawable()).getBitmap();
  ...
  private void onMagicModeChosen() {
    showProgressDialog();
    new ModeChangingTask(MODE_MAGIC, documentBitmap).executeOnExecutor(
        AsyncTask.THREAD_POOL_EXECUTOR);
  }

  private void onBlackAndWhiteModeChosen() {
    showProgressDialog();
    new ModeChangingTask(MODE_BLACK_AND_WHITE, documentBitmap).executeOnExecutor(
        AsyncTask.THREAD_POOL_EXECUTOR);
  }
  ...
  private void onRotateButtonClicked() {
    showProgressDialog();
    new RotatingTask(takenPhotoBitmap, documentBitmap, documentColoredBitmap).executeOnExecutor(
        AsyncTask.THREAD_POOL_EXECUTOR);
  }
  ...
  private void onDoneButtonClicked() {
    if (isCropMode) {
      ...
      if (isScanPointsValid(points)) {
        upScalePoints(points, takenPhotoBitmap, viewHolder.sourceImageView.getWidth(),
            viewHolder.sourceImageView.getHeight());
        new DocumentFromBitmapTask(takenPhotoBitmap, points, currentMode, false).executeOnExecutor(
            AsyncTask.THREAD_POOL_EXECUTOR);
      } else {
        showErrorDialog();
      }
  ...
  private void onPhotoTaken() {
    ...
    DocumentFromBitmapTask documentFromBitmapTask =
        new DocumentFromBitmapTask(takenPhotoBitmap, null, currentMode, true);
  ...
  private static void upScalePoints(Map<Integer, PointF> points, Bitmap original,
      int scaledImgWidth, int scaledImgHeight) {
  ...
  private static Map<Integer, PointF> downScalePoints(Map<Integer, PointF> points, Bitmap original,
      int targetImgWidth, int targetImgHeight) {
    int width = original.getWidth();
    int height = original.getHeight();
  ...
  private static Map<Integer, PointF> orderedValidEdgePoints(Bitmap tempBitmap,
      List<PointF> pointFs) {
  ...
  protected void dismissDialog() {
    if (getActivity() != null && getActivity().getSupportFragmentManager() != null) {
      ProgressDialogFragment progressDialogFragment =
          (ProgressDialogFragment) getActivity().getSupportFragmentManager()
  ...
  private void onDocumentFromBitmapTaskFinished(DocumentFromBitmapTaskResult result,
    ...
    Bitmap scaledBitmap = ImageResizer.scaleBitmap(tmp, viewHolder.sourceFrame.getWidth(),
        viewHolder.sourceFrame.getHeight());
    viewHolder.sourceImageView.setImageBitmap(scaledBitmap);
    viewHolder.sourceImageView.setVisibility(View.INVISIBLE);
    if (correctRotation) {
  ...
  private void onRotatingTaskFinished(RotatingTaskResult rotatingTaskResult) {
    ...
    Bitmap scaledBitmap = ImageResizer.scaleBitmap(
        documentColoredBitmap != null ? documentColoredBitmap : documentBitmap,
        viewHolder.sourceFrame.getWidth(), viewHolder.sourceFrame.getHeight());
    viewHolder.sourceImageView.setImageBitmap(scaledBitmap);
    viewHolder.sourceImageView.setVisibility(View.INVISIBLE);
    viewHolder.scaleImageView.setImage(ImageSource.bitmap(scaledBitmap));
    viewHolder.scaleImageView.setVisibility(View.VISIBLE);
  ...
  private void onCropTaskFinished(CropTaskResult cropTaskResult) {
    ...
    if (points == null) {
      ...
      pointsToUse =
          downScalePoints(points, takenPhotoBitmap, tempBitmap.getWidth(), tempBitmap.getHeight());
    }
  ...
  private class DocumentFromBitmapTask extends AsyncTask<Void, Void, DocumentFromBitmapTaskResult> {
    ...
    @Override protected DocumentFromBitmapTaskResult doInBackground(Void... params) {
      System.gc();
    ...
    protected void onPostExecute(DocumentFromBitmapTaskResult documentFromBitmapTaskResult) {
      onDocumentFromBitmapTaskFinished(documentFromBitmapTaskResult, correctRotation);
      if (correctRotation) {
        new RotatingTask(takenPhotoBitmap, documentBitmap, documentColoredBitmap).executeOnExecutor(
            AsyncTask.THREAD_POOL_EXECUTOR);
      } else {
  ...
  private class RotatingTask extends AsyncTask<Void, Void, RotatingTaskResult> {
    ...
    RotatingTask(Bitmap takenPhotoBitmap, Bitmap documentBitmap, Bitmap documentColoredBitmap) {
      this.takenPhotoBitmap = takenPhotoBitmap;
      this.documentBitmap = documentBitmap;
      this.documentColoredBitmap = documentColoredBitmap;
    }

    @Override protected RotatingTaskResult doInBackground(Void... params) {
    ...
    @Override protected void onPostExecute(RotatingTaskResult rotatingTaskResult) {
  ...
  private class ModeChangingTask extends AsyncTask<Void, Void, ModeChangingTaskResult> {
    ...
    @Override protected ModeChangingTaskResult doInBackground(Void... params) {
    ...
    @Override protected void onPostExecute(ModeChangingTaskResult modeChangingTaskResult) {
  ...
  private class CropTask extends AsyncTask<Void, Void, CropTaskResult> {
    ...
    @Override protected CropTaskResult doInBackground(Void... params) {
      CropTaskResult result = new CropTaskResult();
      try {
        Bitmap scaledBmp = ImageResizer.resizeImage(bitmap, 400, 400, false);
        result.points = getEdgePoints(scaledBmp);
    ...
    @Override protected void onPostExecute(CropTaskResult cropTaskResult) {
  ...
  private static class ViewHolder {
    ...
    void prepare(View parent) {
      sourceImageView = parent.findViewById(R.id.sourceImageView);
      scaleImageView = parent.findViewById(R.id.scaleImage);
      sourceFrame = parent.findViewById(R.id.sourceFrame);
      polygonView = parent.findViewById(R.id.polygonView);
    }
  }
}
